...
#   PALETTES_DIR          palettes repo root (default: parent of scripts/)
#
# Optional:
...
PALETTES_DIR="${PALETTES_DIR:-$ROOT}"
LIB_DIR="${MIP_DIR:-${MICROPYTHON_LIB_DIR:-$PALETTES_DIR/../mip}}"
...
python3 "$PALETTES_DIR/scripts/build.py" \
    --lib-dir "$LIB_DIR" \
    --micropython "$MPY_DIR" \
    --mpy-cross "$MPY_CROSS" \
    --output "$INDEX_OUT"
...
git commit -m "palettes: Update MIP index from PyDevices/palettes ${SHA}."
