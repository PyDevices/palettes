#!/usr/bin/env bash
# Sync palettes into PyDevices/mip, build a pure-Python TestPyPI wheel,
# and push the MIP index.
#
# CI: MICROPYTHON_LIB_DIR=../micropython-lib ./scripts/publish_mip.sh --push
# MIP: mip.install("palettes", index="https://PyDevices.github.io/mip")
...
Copy palettes lib/ into mip, optionally upload TestPyPI wheels,
then commit (and optionally push) on the PyDevices branch.

Options:
  --skip-pypi           Sync manifests only; skip hatch/twine TestPyPI uploads.
  --version X.Y.Z       Release version (overrides tag / PALETTES_VERSION).
...
Environment:
  ...
  PALETTES_VERSION     Release version (overrides git tag on current commit)
...
resolve_version() {
    ...
    if [[ -n "${PALETTES_VERSION:-}" ]]; then
        normalize_version "$PALETTES_VERSION"
        return
    fi
    local tag
    ...
    echo "Error: no release version. Tag HEAD (vX.Y.Z), pass --version, or set PALETTES_VERSION." >&2
    return 1
}
...
# Retire the legacy pydisplay/palettes tree after extraction.
LEGACY_DIR="$DEST_REPO/micropython/pydisplay/palettes"
